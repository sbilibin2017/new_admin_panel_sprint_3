...
def get_filmwork_query(current_state: datetime) -> Tuple[str, datetime]:
...
def get_filmwork_4idxs(chunk: List[Dict]) -> pd.DataFrame:
...
def get_filmwork_genre_4idxs(cursor: psycopg2.extensions.cursor, df: pd.DataFrame) -> pd.DataFrame:
...
def get_filmwork_person_4idxs(cursor: psycopg2.extensions.cursor, df):
